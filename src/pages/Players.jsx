...
export default function Players() {
  ...
  const [showDetailFor, setShowDetailFor] = useState(null); // playerId dont le détail bonus est déplié
  const [recent, setRecent] = useState({});
  const [crests, setCrests] = useState({});

  async function load() {
    const playerList = await fetchPlayerLeaderboard();
    setPlayers(playerList);
    setPageCache(CACHE_KEY, { players: playerList });
    // Les matchs "récents" affichés peuvent être devenus obsolètes
    // (nouveau verrouillage, etc.) — recalculés à la prochaine ouverture.
    setRecent({});
  }
  ...
  function toggleOpen(player) {
    ...
    if (!recent[player.id]) {
      fetchPlayerRecentPredictions(player.id).then((predictions) => {
        setRecent((prev) => ({ ...prev, [player.id]: predictions }));
      });
    }

    // Charge les écussons du Top 8 de ce joueur (déjà connu, pas besoin de refetch).
    (player.bonusPicks?.top8 || []).filter(Boolean).forEach((clubName) => {
          ...
          >
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 9.5, color: 'var(--navy-soft)' }}>
                ...
                {player.bonusTotal > 0 && ` · +${player.bonusTotal} bonus`}
              </div>
              {player.message && (
          ...
          {openId === player.id && (
            <div style={{ background: '#F2F5FC', padding: '10px 18px 16px' }}>
              ...
              <div style={{ display: 'flex', gap: 8, margin: '4px 0 12px' }}>
                ...
                <StatPill label="Bonus" value={`${player.bonusTotal >= 0 ? '+' : ''}${player.bonusTotal}`} highlight />
              </div>

              <button
                onClick={() => setShowDetailFor((prev) => (prev === player.id ? null : player.id))}
                style={{
              ...
              >
                {showDetailFor === player.id ? 'Masquer le détail des bonus ▲' : 'Voir le détail des bonus ▾'}
              </button>

              {showDetailFor === player.id && (
                <div style={{ marginBottom: 14 }}>
                  {player.bonusBreakdown.breakdown.map((item) => (
              ...
              {player.bonusPicks && (player.bonusPicks.top8?.some(Boolean) || player.bonusPicks.topScorer) && (
                <div style={{ marginBottom: 14 }}>
                  ...
                  {player.bonusPicks.top8?.filter(Boolean).length > 0 && (
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 8 }}>
                      {player.bonusPicks.top8.map((clubName, rank) => clubName && (
                  ...
                  {player.bonusPicks.topScorer && (
                    <div style={{ fontSize: 12.5 }}>
                      <span style={{ color: 'var(--navy-soft)' }}>Meilleur buteur : </span>
                      <b>{player.bonusPicks.topScorer}</b>
                    </div>
                  )}
                </div>
              )}
